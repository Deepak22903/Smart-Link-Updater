...
WP_SITES_JSON = os.getenv(
    "WP_SITES"
)  # JSON map of site_key -> {base_url, username, app_password}
...
def _auth_header(wp_site: Optional[Dict] = None) -> dict:
    ...
    if site:
        ...
        logging.info(
            f"[WP] Using site config for auth: base_url={site.get('base_url')}, username={username}"
        )
    else:
        username = WP_USERNAME
        password = WP_APPLICATION_PASSWORD
        logging.info(
            f"[WP] Using environment for auth: base_url={WP_BASE_URL}, username={username}"
        )
...
async def update_post_links_section(
    post_id: int,
    links: List[Link],
    ...
    **Insertion Logic:**
    ...
    timeout = httpx.Timeout(
        30.0, connect=20.0
    )  # 30s read, 20s connect (increased from 10s)
    max_retries = 3
    ...
    for attempt in range(max_retries):
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                r = await client.get(
                    f"{base_url}/wp-json/wp/v2/posts/{post_id}", headers=auth_headers
                )
                r.raise_for_status()
                post = r.json()
                content = post.get("content", {}).get("raw") or post.get(
                    "content", {}
                ).get("rendered", "")
                is_page = False
            ...
            if attempt < max_retries - 1:
                ...
                logging.warning(
                    f"[WP] Timeout on attempt {attempt + 1}/{max_retries} for GET post {post_id}: {e}. Retrying in {wait_time}s..."
                )
                await asyncio.sleep(wait_time)
            else:
                logging.error(
                    f"[WP] Failed to GET post {post_id} after {max_retries} attempts: {e}"
                )
                raise HTTPException(
                    status_code=504,
                    detail=f"WordPress connection timeout after {max_retries} attempts",
                )
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 404:
                logging.warning(
                    f"[WP] Post {post_id} not found (404), attempting to fetch as page..."
                )
                try:
                    async with httpx.AsyncClient(timeout=timeout) as client:
                        r = await client.get(
                    ...
                    raise HTTPException(
                        status_code=404,
                        detail=f"Neither post nor page with ID {post_id} found",
                    )
            else:
    ...
    if not links:
        return {"sections_pruned": 0, "links_added": 0, "updated": False}
    ...
    logging.info(
        f"[WP] Keeping sections from last {days_to_keep} days (cutoff: {cutoff_date.date()})"
    )
    ...
    logging.info(
        f"[WP DEBUG] 'smartlink-updater-section' in content: {'smartlink-updater-section' in content}"
    )
    ...
    old_section_pattern = (
        r'<div class="links-for-today"[^>]*>[\s\S]*?<p[^>]*>.*?</p>\s*</div>'
    )
    ...
    def should_keep_section(match_text, section_date_str):
        ...
        if section_date_str == formatted_date:
            logging.info(
                f"[WP PRUNING] Section '{section_date_str}' is today - will be rebuilt"
            )
        ...
        try:
            ...
            if should_keep:
                logging.info(
                    f"[WP PRUNING] KEEPING section '{section_date_str}' ({days_old} days old)"
                )
            else:
                logging.info(
                    f"[WP PRUNING] REMOVING section '{section_date_str}' ({days_old} days old - older than {days_to_keep} days)"
                )

            return should_keep
        except Exception as e:
            logging.warning(
                f"[WP PRUNING] Failed to parse date '{section_date_str}': {e} - keeping section"
            )
    ...
    logging.info(
        f"[WP DEBUG] Found {len(new_block_matches)} sections with new_block_pattern"
    )
    ...
    logging.info(
        f"[WP DEBUG] Found {len(old_format_matches)} sections with old_section_pattern"
    )
    ...
    for match in old_format_matches:
        ...
        if date_match:
            ...
            if section_date_str == formatted_date:
                if (
                    not today_section_exists
                ):  # Only process if not already found in new format
                    today_section_exists = True
    ...
    logging.info(
        f"[WP DEBUG] After processing: had_sections={had_sections}, sections_to_keep_count={sections_to_keep_count}, sections_to_remove={len(sections_to_remove)}"
    )
    ...
    if sections_to_remove:
        logging.info(
            f"[WP] Removing {len(sections_to_remove)} old sections (keeping last {days_to_keep} days)"
        )
    ...
    use_custom_title = (
        post_config.get("use_custom_button_title", False) if post_config else False
    )
    custom_title = post_config.get("custom_button_title", None) if post_config else None

    logging.info(
        f"[WP] Custom button title config: enabled={use_custom_title}, title={custom_title}"
    )
    ...
    logging.info(
        f"[WP] Using button style: {button_style}, numbering mode: {button_numbering}"
    )
...
<!-- /wp:group -->"""
    ...
    matches_p1 = list(
        re.finditer(ad_code_pattern_1, cleaned_content, re.DOTALL | re.IGNORECASE)
    )
    ...
    if not matches_p1:
        matches_p2 = list(
            re.finditer(ad_code_pattern_2, cleaned_content, re.DOTALL | re.IGNORECASE)
        )
    ...
    if len(existing_ad_matches) > 0:
        ...
        if matches_p1:
            cleaned_content = re.sub(
                ad_code_pattern_1, "", cleaned_content, flags=re.DOTALL | re.IGNORECASE
            )
        else:
            cleaned_content = re.sub(
                ad_code_pattern_2, "", cleaned_content, flags=re.DOTALL | re.IGNORECASE
            )

        logging.info(f"[WP] Removed {len(existing_ad_matches)} existing ad code blocks")

        # Verify removal - check both patterns
        remaining_ads_p1 = list(
            re.finditer(ad_code_pattern_1, cleaned_content, re.DOTALL | re.IGNORECASE)
        )
        remaining_ads_p2 = list(
            re.finditer(ad_code_pattern_2, cleaned_content, re.DOTALL | re.IGNORECASE)
        )
        ...
        if total_remaining > 0:
            logging.warning(
                f"[WP] WARNING: {total_remaining} ad blocks still remain after removal!"
            )
    ...
    if sections_to_keep_count > 0:
        ...
        existing_sections = list(
            re.finditer(smartlink_block_pattern, cleaned_content, re.DOTALL)
        )
        ...
        if existing_sections:
            ...
            logging.info(
                f"[WP] Inserted new SmartLink section before existing sections ({sections_to_keep_count} sections remaining)"
            )
        else:
            # Fallback: should not happen if sections_to_keep_count > 0
            logging.warning(
                f"[WP] Expected {sections_to_keep_count} sections but found none - inserting after H2"
            )
            h2_block_pattern = r"(<!-- wp:heading.*?-->.*?<h2[^>]*>.*?</h2>.*?<!-- /wp:heading -->|<h2[^>]*>.*?</h2>)"
            h2_match = re.search(
                h2_block_pattern, cleaned_content, re.DOTALL | re.IGNORECASE
            )
        ...
        logging.info(
            f"[WP] All sections were for today and removed - inserting new merged section after first H2"
        )
        h2_block_pattern = r"(<!-- wp:heading.*?-->.*?<h2[^>]*>.*?</h2>.*?<!-- /wp:heading -->|<h2[^>]*>.*?</h2>)"
        h2_match = re.search(
            h2_block_pattern, cleaned_content, re.DOTALL | re.IGNORECASE
        ...
        if h2_match:
            ...
            new_content = new_section + "\n\n" + cleaned_content
            logging.warning(
                f"[WP] No <h2> block found, prepending SmartLink block to beginning"
            )
    else:
        # No SmartLink sections ever existed - this is a fresh post
        # Insert after first H2 block (WordPress heading block)
        logging.info(
            f"[WP] No SmartLink sections found - inserting after first H2 (fresh post)"
        )
        h2_block_pattern = r"(<!-- wp:heading.*?-->.*?<h2[^>]*>.*?</h2>.*?<!-- /wp:heading -->|<h2[^>]*>.*?</h2>)"
        h2_match = re.search(
            h2_block_pattern, cleaned_content, re.DOTALL | re.IGNORECASE
    ...
    if post_config:
        ...
        if target_site_key and target_site_key in all_site_ads:
            ...
            logging.info(
                f"[WP] Using site-specific ad codes for site '{target_site_key}': {len(ad_codes)} ads"
            )
    ...
    if ad_codes and isinstance(ad_codes, list):
        ...
        all_sections = list(
            re.finditer(smartlink_block_pattern, new_content, re.DOTALL)
        )
        ...
        if all_sections:
            ...
            button_rows = list(
                re.finditer(button_rows_pattern, today_section_content, re.DOTALL)
            )

            logging.info(
                f"[WP] Found {len(button_rows)} button rows in today's section"
            )

            # Process ad codes in reverse order to maintain correct positions
            ad_codes_sorted = sorted(
                ad_codes, key=lambda x: int(x.get("position", 1)), reverse=True
            )

            for ad_code_config in ad_codes_sorted:
                position = ad_code_config.get(
                    "position", 1
                )  # Row number after which to insert (1-based)
                ...
                try:
                    ...
                    logging.warning(
                        f"[WP] Invalid position value: {position}, defaulting to 1"
                    )
                    position = 1
                ...
                if row_index < len(button_rows):
                    ...
                    absolute_insertion_point = (
                        today_section_start + relative_insertion_point
                    )
...
"""
                    ...
                    new_content = (
                        new_content[:absolute_insertion_point]
                        + ad_html
                        + new_content[absolute_insertion_point:]
                    )
                    logging.info(
                        f"[WP] Inserted ad after button row {position} (row index {row_index})"
                    )
                    ...
                    if all_sections:
                        ...
                        button_rows = list(
                            re.finditer(
                                button_rows_pattern, today_section_content, re.DOTALL
                            )
                        )
                else:
                    logging.warning(
                        f"[WP] Cannot insert ad after row {position} - only {len(button_rows)} rows exist"
                    )
        else:
    ...
    timeout = httpx.Timeout(
        60.0, connect=20.0
    )  # 60s read for POST (can be slow), 20s connect (increased from 10s)
    max_retries = 3
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logging.warning(
                    f"[WP] Timeout on attempt {attempt + 1}/{max_retries} for POST {content_type} {post_id}: {e}. Retrying in {wait_time}s..."
                )
                await asyncio.sleep(wait_time)
            else:
                logging.error(
                    f"[WP] Failed to POST {content_type} {post_id} after {max_retries} attempts: {e}"
                )
                raise HTTPException(
                    status_code=504,
                    detail=f"WordPress connection timeout after {max_retries} attempts",
            ...
            logging.error(
                f"[WP] Unexpected error posting to {content_type} {post_id}: {e}"
            )
            raise
    ...
    return {
        ...
        "new_links_added": len(
            [
                url
                for url in [link.url for link in links]
                if url not in [el["url"] for el in existing_links]
            ]
        ),
...
async def update_post_promo_codes_section(
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                try:
                    ...
                    logging.error(
                        f"[WP PROMO] Failed to GET page {post_id}: {page_error}"
                    )
                    raise HTTPException(
                        status_code=404,
                        detail=f"Neither post nor page with ID {post_id} found",
            ...
            if attempt < max_retries - 1:
                ...
                logging.warning(
                    f"[WP PROMO] Timeout on attempt {attempt + 1}/{max_retries}: {e}. Retrying..."
                )
                await asyncio.sleep(wait_time)
            else:
                raise HTTPException(
    ...
    now = datetime.now(pytz.timezone("UTC"))
    today_date = (
        promo_codes[0].published_date_iso if promo_codes else now.strftime("%Y-%m-%d")
    )
    ...
    for idx, code_data in enumerate(merged_codes, start=1):
        code_items.append(
            f"<li style=\"margin: 10px 0; font-family: 'Courier New', Consolas, monospace; font-size: 16px; font-weight: 600; color: #111827;\">{code_data['code']}</li>"
        )
...
<!-- /wp:html -->"""
    ...
    if promo_match:
        ...
        new_content = (
            cleaned_content[: promo_match.start()]
            + new_section
            + cleaned_content[promo_match.end() :]
        )
    else:
    ...
    for attempt in range(max_retries):
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                r.raise_for_status()
                logging.info(
                    f"[WP PROMO] Successfully updated post {post_id} with {len(merged_codes)} promo codes"
                )
                break
        except (httpx.ConnectTimeout, httpx.ReadTimeout) as e:
            if attempt < max_retries - 1:
                ...
                raise HTTPException(
                    status_code=504, detail="WordPress connection timeout"
                )
        except httpx.HTTPStatusError as e:
