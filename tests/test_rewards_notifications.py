...
def test_is_notifications_enabled_truthy_values():
    assert (
        rewards_notifications.is_notifications_enabled({"send_notifications": True})
        is True
    )
    assert (
        rewards_notifications.is_notifications_enabled({"send_notifications": "true"})
    ...
    assert (
        rewards_notifications.is_notifications_enabled({"send_notifications": "1"})
        is True
    )
    assert (
        rewards_notifications.is_notifications_enabled({"send_notifications": "yes"})
        is True
    )
...
def test_is_notifications_enabled_falsy_values():
    assert (
        rewards_notifications.is_notifications_enabled({"send_notifications": False})
        is False
    )
    assert (
        rewards_notifications.is_notifications_enabled({"send_notifications": "false"})
    ...
    assert (
        rewards_notifications.is_notifications_enabled({"send_notifications": 0})
        is False
    )
...
async def test_notify_rewards_update_filters_by_app_id(monkeypatch):
    ...
    async def fake_notify_new_rewards(
        push_tokens_dict, count=None, title=None, body=None, app_id=None
    ):
    ...
    monkeypatch.setattr(
        rewards_notifications.mongo_storage, "list_push_tokens", fake_list_push_tokens
    )
    monkeypatch.setattr(
...
@pytest.mark.asyncio
async def test_notify_rewards_update_keeps_domino_rewards_mapping(monkeypatch):
    sent = {}
    ...
    assert sent["count"] == 4
    assert sent["app_id"] == "domino_rewards"
